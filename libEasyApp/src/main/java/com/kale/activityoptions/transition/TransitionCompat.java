...
import com.kale.activityoptions.ActivityOptionsCompatHelper;
...
public class TransitionCompat {
    ...
    private static void endBitmapAnimation(Activity activity) {
         ...
         * 开始设定view开始时的大小和坐标位置
         */
        LayoutParams orginalParams = new LayoutParams(ActivityOptionsCompatHelper.getScreenWidth(activity),
                ActivityOptionsCompatHelper.getScreenHeight(activity));
        ...
        boolean isFinalFullScreen = ActivityOptionsCompatHelper.isFullScreen(activity);
        if (mIsStartFullScreen == false && isFinalFullScreen == true) {
            finalOffsetY = ActivityOptionsCompatHelper.getStatusBarHeight(activity);
        } else {
            finalOffsetY = 0;
        }
    ...
    private static void endSharedElementAnimation(final Activity activity, int layoutResId) {
        ...
        if (!ActivityOptionsCompatHelper.isInScreen(activity, currentView)) {
        ...
        ViewHelper.setX(orginalView, orginalBounds.left);
        ViewHelper.setY(orginalView, orginalBounds.top);
        /**
         * 获得目标view的显示范围
         */
    ...
    public static void finishAfterTransition(Activity activity) {
        ...
        isEnter = false;
        //        activity.setResult(ActivityOptionsCompatHelper.RESULT_CODE);
        mListener = new TransitionCompat().new MyTransitionListener();
        //开始执行动画，这里的false表示执行结束的动画
        switch (mAnimationType) {

            case ActivityOptionsCompatHelper.ANIM_SCALE_UP:
                scaleUpAnimation(activity, false);
                break;
            case ActivityOptionsCompatHelper.ANIM_THUMBNAIL_SCALE_UP:
                thumbnailScaleUpAnimation(activity, false);
                break;
            case ActivityOptionsCompatHelper.ANIM_SCENE_TRANSITION:
                sceneTransitionAnimation(activity, mLayoutResId, false);
                break;
            case ActivityOptionsCompatHelper.ANIM_NONE:
                activity.finish();
                return;
            default:
                activity.finish();
    ...
    private static View getSharedElementOrginalView(Activity activity, int layoutResId) {
        ...
        if (mIsVerticalScreen != ActivityOptionsCompatHelper.isVerticalScreen(activity)) {
            return null;
        }
    ...
    private static void scaleUpAnimation(final Activity activity, final boolean isEnter) {
        ...
        //执行屏幕的动画
        if (mTransitionAnims == null) {
            final SceneScaleUp anim = new SceneScaleUp(activity, mStartX, mStartY, mWidth, mHeight);
    ...
    public static void setTimeInterpolator(Interpolator interpolator) {
    ...
    private static void startBitmapAnimation(Activity activity) {
        ...
        final Rect finalBounds = new Rect(0, 0, (int) (ActivityOptionsCompatHelper.getScreenWidth(activity) * fraction),
                (int) (ActivityOptionsCompatHelper.getScreenHeight(activity) * fraction));
        /**
         * 这里可以设置动画的持续时间，开始延迟，添加监听器
         */
        final ViewAnim anim = new ViewAnim();
    ...
    private static void startSharedElementAnimation(final Activity activity, int layoutResId) {
        ...
        orginalView.post(new Runnable() {
            ...
            public void run() {
                ...
                if (!ActivityOptionsCompatHelper.isInScreen(activity, targetView)) {
    ...
    public static void startTransition(final Activity activity, final int layoutResId) {
        ...
        mListener = new TransitionCompat().new MyTransitionListener();
        mBundle = bundle;
        mAnimationType = bundle.getInt(ActivityOptionsCompatHelper.KEY_ANIM_TYPE, ActivityOptionsCompatHelper.ANIM_NONE);
        mIsVerticalScreen = bundle.getBoolean(ActivityOptionsCompatHelper.KEY_IS_VERTICAL_SCREEN);
        /**
        ...
        switch (mAnimationType) {
            ...
            case ActivityOptionsCompatHelper.ANIM_SCALE_UP:
                mIsInTheScreen = bundle.getBoolean(ActivityOptionsCompatHelper.KEY_IS_IN_THE_SCREEN);
                mWidth = bundle.getInt(ActivityOptionsCompatHelper.KEY_ANIM_WIDTH);
                mHeight = bundle.getInt(ActivityOptionsCompatHelper.KEY_ANIM_HEIGHT);
                mStartX = bundle.getInt(ActivityOptionsCompatHelper.KEY_ANIM_START_X);
                mStartY = bundle.getInt(ActivityOptionsCompatHelper.KEY_ANIM_START_Y);
                //开始执行动画，这里的tue表示是开始状态
                scaleUpAnimation(activity, true);
                break;

            case ActivityOptionsCompatHelper.ANIM_THUMBNAIL_SCALE_UP:
                mIsStartFullScreen = bundle.getBoolean(ActivityOptionsCompatHelper.KEY_IS_START_FULL_SCREEN);
                mThumbnail = (Bitmap) bundle.getParcelable(ActivityOptionsCompatHelper.KEY_ANIM_THUMBNAIL);
                mIsInTheScreen = bundle.getBoolean(ActivityOptionsCompatHelper.KEY_IS_IN_THE_SCREEN);
                mWidth = bundle.getInt(ActivityOptionsCompatHelper.KEY_ANIM_WIDTH);
                mHeight = bundle.getInt(ActivityOptionsCompatHelper.KEY_ANIM_HEIGHT);
                mStartX = bundle.getInt(ActivityOptionsCompatHelper.KEY_ANIM_START_X);
            ...
            case ActivityOptionsCompatHelper.ANIM_SCENE_TRANSITION:
                mIsStartFullScreen = bundle.getBoolean(ActivityOptionsCompatHelper.KEY_IS_START_FULL_SCREEN);
                mIsInTheScreenArr = bundle.getBooleanArray(ActivityOptionsCompatHelper.kEY_IS_IN_THE_SCREEN_ARR);
                mSharedElementIds = bundle.getIntegerArrayList(ActivityOptionsCompatHelper.kEY_SHARED_ELEMENT_ID_LIST);
                mSharedElementBounds =
                        bundle.getParcelableArrayList(ActivityOptionsCompatHelper.kEY_SHARED_ELEMENT_BOUNDS_LIST);
