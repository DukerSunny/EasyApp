...
public class ActivityOptionsCompatHelper {
     ...
     */
    public static ActivityOptionsCompatHelper makeCustomAnimation(Context context, int enterResId, int exitResId) {

        ActivityOptionsCompatHelper opts = new ActivityOptionsCompatHelper();
        // 设置动画类型和动画资源的id
     ...
     */
    public static ActivityOptionsCompatHelper makeScaleUpAnimation(View source, int startX, int startY, int width, int height) {
    ...
    public static ActivityOptionsCompatHelper makeSceneTransitionAnimation(Activity activity, final View sharedElement,
            int sharedElementId) {
     ...
     * 这个方法必须放在view加载完后再执行，否则获取不到view的宽度和坐标
     *
     * @param sharedElements：执行动画的元素，新的activity中要接收动画效果的元素的id，这样就得到了两个view之间的联系
     *         通过两点一线的方式来进行补间动画。
     *
     ...
     *android.transition.Transition.EpicenterCallback)
     */
    public static ActivityOptionsCompatHelper makeSceneTransitionAnimation(Activity activity,
     ...
     * supply these options as the options Bundle when starting an activity.
     */
    public static ActivityOptionsCompatHelper makeThumbnailScaleUpAnimation(View source, Bitmap thumbnail, int startX,
            int startY) {
        Activity activity = (Activity) source.getContext();
        ActivityOptionsCompatHelper opts = new ActivityOptionsCompatHelper();
