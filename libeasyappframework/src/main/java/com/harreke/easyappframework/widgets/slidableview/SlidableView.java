...
import com.harreke.easyappframework.widgets.slidableview.inners.OverlayView;
import com.harreke.easyappframework.widgets.slidableview.inners.TabView;
...
public class SlidableView extends FrameLayout implements ViewGroup.OnHierarchyChangeListener, OnGestureListener {
    ...
    private Runnable mDelayedRefreshCompleteRunnable = new Runnable() {
        @Override
        public void run() {
            mAnimate.start(mTabsHeight);
        }
    };

    public SlidableView(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.slidingViewStyle);
    }
    ...
    private void calculateRefreshTrigger() {
        ...
        int size = mTabsHeight + mHeadersHeight;
        int progress;
        int i;
        /**
        ...
        for (i = 0; i < mHeaders.getChildCount(); i++) {
            if (mHeaders.getChildAt(i) instanceof RefreshHeaderView) {
                ...
                progress = (int) (100 * (mOffset - size) / mHeadersHeight);
        ...
        if (!mRefreshing || mOffset > mHeadersHeight) {
            ...
            if (scrollY > 0f) {
                ...
                if (mContent.isSlidableTouchTop()) {
                    ...
                    //                    DevUtil.e("targetOffset=" + targetOffset + " mBarHeight=" + mBarHeight + " mStartOffset=" + mStartOffset + " scrollY=" + scrollY +
                    //                            " offsetScale=" + offsetScale);
                    //                    DevUtil.e("scroll=" + scrollY * offsetScale);
        ...
        if (!mRefreshing) {
            if (mOffset > mBarHeight) {
                if (mSlidableTriggerListener != null) {
                    mSlidableTriggerListener.onRefreshStart();
                }
            } else if (mOffset > mTabsHeight) {
                mAnimate.start(mTabsHeight);
            } else {
                mAnimate.start(mBaseOffset);
            }
        } else {
            mAnimate.start(mBarHeight);
        }
    }
    ...
    public final void setDelayAfterComplete(int delay) {
        if (delay >= 0) {
            mDelay = delay;
        }
    }
    ...
    public final void setLoadComplete(int pageSize) {
        if (mLoading) {
            ...
            postDelayed(mDelayedLoadCompleteRunnable, mDelay);
        }
    }
    ...
    public final void setLoadStart() {
        ...
        if (!mLoading) {
            mLoading = true;
            mAnimateOverlay.start(mOverlaysHeight);
            /**
             * 遍历Overlay，如果有LoadOverlay就设置其加载状态
             */
            for (i = 0; i < mOverlays.getChildCount(); i++) {
     ...
     *         偏移等级
     *
     * @see com.harreke.easyappframework.widgets.slidableview.inners.HeaderView
     * @see com.harreke.easyappframework.widgets.slidableview.inners.TabView
     */
    ...
    public final void setRefreshComplete() {
        ...
        if (mRefreshing) {
            ...
            postDelayed(mDelayedRefreshCompleteRunnable, mDelay);
        }
    }
     ...
     *
     * @param slidableViewId
     *         可滑动内容视图Id
     */
    public final void setSlidableView(int slidableViewId) {
        if (mContent != null && slidableViewId > 0) {
            mContent.setSlidableView(slidableViewId);
        }
    }
    ...
    private class Animate implements Runnable {
        ...
        public final void start(float targetOffset) {
    ...
    private class AnimateOverlay implements Runnable {
        private float mAnimateDistance = 0;
        private int mAnimateInterval = 0;
        private float mAnimateStep = 0f;
        private boolean mAnimating = false;
        private int mTargetCount;
        ...
        public final void start(float targetOffset) {
            mTargetOffset = targetOffset;
            mTargetCount = ANIMATE_COUNT;
            mAnimateStep = mTargetOffset / mTargetCount;
            mAnimateInterval = ANIMATE_DURATION / mTargetCount;
            mAnimateDistance = 0;
            mAnimating = true;
            postDelayed(this, mAnimateInterval);
        ...
        public void run() {
            if (mAnimating) {
                if (mTargetCount > 0) {
                    mAnimateDistance += mAnimateStep;
                    mOverlayOffset = mOffset + mAnimateDistance;
                } else {
                    mOverlayOffset = mOffset + mTargetOffset;
                    mAnimating = false;
                }
                refresh();
                mTargetCount--;
